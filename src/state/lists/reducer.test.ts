  ...
  describe('updateVersion', () => {
    describe('never initialized', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            'https://unpkg.com/@eliteswap/default-token-list@latest/uniswap-default.tokenlist.json': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
      ...
      it('clears the current lists', () => {
        expect(
          store.getState().byUrl['https://unpkg.com/@eliteswap/default-token-list@latest/uniswap-default.tokenlist.json']
        ).toBeUndefined()
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          selectedListUrl: undefined,
          lastInitializedDefaultListOfLists: ['https://unpkg.com/@eliteswap/default-token-list@latest']
        })
        store.dispatch(updateVersion())
      })
      ...
      it('each of those initialized lists is empty', () => {
        ...
        Object.keys(byUrl).forEach(url => {
          if (url !== 'https://unpkg.com/@eliteswap/default-token-list@latest/uniswap-default.tokenlist.json') {
